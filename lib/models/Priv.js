...
import {Curve, std_curve, std_curve as stdCurve} from '../curve.js';
  ...
  static from_protected(data, password, algo) {
    ...
    if (password) {
      stores = parseWithFn(data, [
        pbes2.pbes2_parse,
        pfx.pfx_parse,
        keystore.enc_parse_many
      ]);
      data = algo.storeloadall
        ? algo.storeloadall(stores, password)
        : stores.map(part => algo.storeload(part, password));
    } else {
      data = [data];
    }
