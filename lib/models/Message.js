...
class SignedAttrs extends Attrs {
  ...
  set signingCertificateV2(value) {
    this.setAttr(
      ...
      certid.SigningCertificateV2.wrap(
        value.cert.ob,
        value.hash,
        value.hashAlgorithm
      )
...
class Message {
  ...
  verifyTimestampToken(msg, hash_f, lookupCert, lookupCA, imprintOf, hashes) {
    ...
    if (
      token.messageImprint.hashAlgorithm.algorithm !==
      (hash_f.algo || "Gost34311")
    ) {
      return false;
    }
  ...
  verify(hash_f, lookupCert, lookupCA, opts = {}) {
    const signatureAlgorithm = this.info.signerInfos[0].digestEncryptionAlgorithm
      .algorithm;
